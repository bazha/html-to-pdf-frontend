...
vi.mock('../api/pdfClient', async () => {
    const actual = await vi.importActual<typeof import('../api/pdfClient')>('../api/pdfClient')
    return {
        ...actual,
        pollJob: vi.fn(),
    }
...
describe('usePoll', () => {
    ...
    it('stops on failed', async () => {
        ...
        await waitFor(() => expect(result.current).toMatchObject({phase: 'failed', reason: 'oops'}))
    })
    ...
    it('times out after max wall-clock', async () => {
        ...
        await waitFor(() => expect(result.current).toMatchObject({phase: 'error', code: 'timeout'}))
    })
})
